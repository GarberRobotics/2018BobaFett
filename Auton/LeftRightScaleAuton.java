...
public class LeftRightScaleAuton extends Command{
	public static double l, r = .7;
	public static double pidAngle, autonZ, oppositeSide;
	...
	public static boolean bet, bet2 = false;
	private AutonFunctions autonFunctions;
	
	public LeftRightScaleAuton(){
	}
	...
	public void LeftScale(){
		...
		}else if(Robot.frontRight.getSelectedSensorPosition(0) < slightlyForward + 3500 && stepFourLeft){
			stepThreeLeft = false;
			autonFunctions.ElevatorUp();
			l = r = .45;
	...
	public void RightScale(){
		if(stepOneLeft){
			...
			if(Robot.imu.getAngleZ() < rightTurn){
				autonFunctions.TurnRight(1.5, 2);
				setSpeed();
				autonFunctions.ArmUp();
			}else if(Robot.frontRight.getSelectedSensorPosition(0) < 0){
				r = .55;
				l = -.55;
				Robot.ArmMotor.set(0);
			}else{
			...
			setSpeed();
			autonFunctions.ArmUp();
			if(Robot.frontRight.getSelectedSensorPosition(0) > oppositeSide + 36000){
				l = r = -.5;
				bet2 = true;
			}
			...
			if(Robot.frontRight.getSelectedSensorPosition(0) < slightlyForward + 7000){
				l = r = .5;
			}else{
				l = r = 0;
				stepSixLeft = false;
